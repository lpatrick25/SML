...
//   }
    function setCaretPosition(elem, caretPos) {
        try {
            ...
            // For modern browsers — only call if it's supported
            else if (typeof elem.setSelectionRange === 'function') {
                elem.setSelectionRange(caretPos, caretPos);
            }
            // Optional: log unsupported element types for debugging
            else {
                console.warn('setCaretPosition: Unsupported element type for setting caret position', elem);
            }
        } catch (ex) {
            console.error('setCaretPosition error:', ex);
        }
    }
